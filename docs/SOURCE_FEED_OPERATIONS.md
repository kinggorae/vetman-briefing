...
현재 source-first 일간 기준은 활성 소스 51개 이상, RSS/Atom 피드를 제공하는 소스 38개 이상(피드 엔드포인트 39개), 후보 풀 260개, 심층 생성 60개다. 공식 학술 피드는 npj Veterinary Sciences, Journal of Research in Veterinary Medicine, American Journal of Traditional Chinese Veterinary Medicine까지 포함한다. 발행은 하루 최대 90건을 목표로 하되, 품질 게이트를 통과한 후보가 30건 이상이면 가용분을 먼저 공개해 하루 전체가 결번이 되지 않게 한다. 30건 미만인 경우에만 제한된 보강 수집을 한 번 더 수행한다.
