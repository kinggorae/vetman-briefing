...
배포·일일 발행·PR 품질 검사는 빌드 전후의 `site/search.json`을 비교합니다.
기존 공개 기사 ID가 다음 빌드에서 사라지거나, 검색 인덱스의 ID가 중복되거나,
선언된 개수와 실제 항목 수가 달라지면 배포가 중단됩니다. 이 검사는 하루 최신
기사 수를 제한하는 기능과 별개이며, 기존 기사와 새 기사를 함께 보존하는 안전망입니다.

의도적인 삭제가 꼭 필요한 경우에는 기사 ID와 승인 근거를 코드 리뷰에 남긴 뒤,
해당 실행에서만 `RETENTION_ALLOW_REMOVALS` 환경변수로 명시합니다. 일반 발행에는
이 변수를 설정하지 않으며, 기존 기사는 기본적으로 삭제할 수 없습니다.
