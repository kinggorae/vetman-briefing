...
API 토큰, OAuth refresh token, 서비스 계정 키, 사용자 검색어 원본에 포함된 개인정보를 저장소·로그·이슈에 남기지 않습니다. API 연동은 `.env.example`에 정의된 변수명을 사용하되 실제 값은 CI secret 또는 로컬 보안 저장소에서만 주입합니다.
