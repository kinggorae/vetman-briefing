...
import { compareDeploymentPayload, inspectDeploymentPayload } from "../src/lib/production-monitor.js";
...
function check(liveDeployment, liveLatest, liveRobots) {
  const critical = [];
...
for (let attempt = 1; attempt <= attempts; attempt += 1) {
  try {
    const [deployment, latest, robots] = await Promise.all([
    ...
    if (!last.length) last.push(...check(deployment.payload, latest.payload, robots.body));
    if (!last.length) {
