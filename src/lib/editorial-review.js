...
export function clinicalReviewIssues(item = {}, { issueDate = "", existingIndex = false } = {}) {
  const status = normalizeEditorialStatus(item.editorialStatus);
  ...
  if (!existingIndex && isPolicyNew(item, issueDate)) {
    if (risk === "high" && status !== "vet-reviewed") issues.push("high-risk-not-vet-reviewed");
    if (risk === "medium" && !["editor-reviewed", "vet-reviewed"].includes(status)) issues.push("medium-risk-not-editor-reviewed");
  }
