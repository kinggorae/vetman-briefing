...
(function(){
  ...
  // ── 레이더(조기경보) 렌더 ──
  function stars(n){ n=Math.max(0,Math.min(4,n||0)); return '★★★★'.slice(0,n)+'☆☆☆☆'.slice(0,4-n); }
  // 논문 근거등급 인라인 배지 (제목/카드 메타 옆)
  function evBadge(a){
    var ev=a.radar&&a.radar.evidence; if(!ev) return '';
    var bits=[ev.design,ev.n].filter(Boolean).map(e).join(' · ');
    return '<span title="근거 수준(별 많을수록 강함)" style="display:inline-flex;align-items:center;gap:5px;padding:3px 8px;border-radius:6px;background:rgba(0,102,255,0.07);border:1px solid var(--color-line-normal);font-size:11px;font-weight:700;color:var(--color-label-neutral);white-space:nowrap;"><span style="letter-spacing:1px;color:var(--color-primary-normal);">'+stars(ev.stars)+'</span>'+(bits?'<span style="color:var(--color-label-alternative);font-weight:600;">'+bits+'</span>':'')+'</span>';
  }
  // "곧 보호자가 물어봄" 칩 (카드용)
  function ownerChip(a){
    var o=a.radar&&a.radar.owner; if(!o||!o.soon) return '';
    return '<span title="곧 보호자가 물어볼 주제" style="display:inline-flex;align-items:center;gap:4px;padding:2px 7px;border-radius:5px;background:var(--color-primary-normal);color:#fff;font-size:10px;font-weight:700;letter-spacing:.02em;white-space:nowrap;">⚑ 보호자 레이더</span>';
  }
  // 카드용 근거 별점만(컴팩트)
  function evMini(a){
    var ev=a.radar&&a.radar.evidence; if(!ev) return '';
    return '<span title="근거 수준: '+e(ev.design||'')+(ev.n?' · '+e(ev.n):'')+'" style="display:inline-flex;align-items:center;padding:2px 6px;border-radius:5px;background:rgba(0,102,255,0.08);font-size:10.5px;font-weight:700;letter-spacing:1px;color:var(--color-primary-normal);white-space:nowrap;">'+stars(ev.stars)+'</span>';
  }
  function chips(a){ return ownerChip(a)+evMini(a); }
  // 상세뷰 레이더 블록 — 진료 점검 + 보호자 설명 스크립트 + 근거 상세
  function radarBlock(a){
    var r=a.radar; if(!r||(!r.clinical&&!r.owner&&!r.evidence)) return '';
    var h='<div style="margin:24px 0 8px;"><div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;"><span style="font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;color:var(--color-primary-normal);">해외 조기경보 레이더</span><span style="flex:1;height:1px;background:var(--color-line-normal);"></span></div>';
    // 진료 레이더
    if(r.clinical){
      h+='<div style="margin-bottom:12px;padding:14px 16px;background:var(--color-background-alternative);border-radius:10px;border-left:3px solid var(--color-label-strong);">'
      +'<div style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--color-label-alternative);margin-bottom:5px;">🩺 진료 레이더 · 이렇게 점검하세요</div>'
      +'<div style="font-size:calc(14.5px*var(--fs));line-height:1.6;color:var(--color-label-strong);font-weight:600;">'+e(r.clinical)+'</div></div>';
    }
    // 보호자 레이더
    if(r.owner){
      var soon=r.owner.soon;
      h+='<div style="margin-bottom:12px;border:1px solid var(--color-primary-normal);border-radius:12px;overflow:hidden;">'
      +'<div style="padding:12px 16px;background:rgba(0,102,255,0.06);display:flex;align-items:center;gap:8px;"><span style="font-size:11px;font-weight:800;letter-spacing:.04em;color:var(--color-primary-normal);">⚑ 보호자 레이더</span><span style="font-size:11.5px;color:var(--color-label-neutral);">'+(soon?'곧 보호자가 이렇게 물어봅니다':'보호자가 물어볼 수 있어요')+'</span></div>'
      +'<div style="padding:14px 16px;">'
      + (r.owner.q?'<div style="font-size:calc(14.5px*var(--fs));line-height:1.55;font-weight:700;color:var(--color-label-strong);margin-bottom:10px;">“'+e(r.owner.q)+'”</div>':'')
      + (r.owner.script?'<div style="font-size:11px;font-weight:700;color:var(--color-label-alternative);margin-bottom:5px;">이렇게 설명하세요</div><div style="font-size:calc(14px*var(--fs));line-height:1.7;color:var(--color-label-neutral);">'+e(r.owner.script)+'</div>':'')
      + (r.owner.script?'<div style="margin-top:12px;"><button data-act="copyscript" data-id="'+a.id+'" style="display:inline-flex;align-items:center;gap:6px;border:1px solid var(--color-line-normal);background:var(--color-background-normal);color:var(--color-label-neutral);cursor:pointer;font-family:inherit;font-size:12.5px;font-weight:700;padding:8px 12px;border-radius:8px;">'+COPY+' 설명 복사</button></div>':'')
      +'</div></div>';
    }
    // 근거 등급 상세 (논문)
    if(r.evidence){
      var ev=r.evidence;
      h+='<div style="margin-bottom:12px;padding:14px 16px;background:var(--color-background-alternative);border-radius:10px;">'
      +'<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;"><div style="font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--color-label-alternative);">📊 근거 등급</div><div style="display:flex;align-items:center;gap:8px;"><span style="letter-spacing:2px;font-size:14px;color:var(--color-primary-normal);">'+stars(ev.stars)+'</span>'+(ev.design?'<span style="font-size:12.5px;font-weight:700;color:var(--color-label-strong);">'+e(ev.design)+'</span>':'')+(ev.n?'<span style="font-size:12px;color:var(--color-label-alternative);">'+e(ev.n)+'</span>':'')+'</div></div>'
      + (ev.note?'<div style="margin-top:8px;font-size:12.5px;line-height:1.55;color:var(--color-label-neutral);">⚠️ '+e(ev.note)+'</div>':'')
      +'</div>';
    }
    return h+'</div>';
  }
  ...
  function leadCard(a){
    ...
    +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap;"><span style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--color-primary-normal);">'+e(a.kicker)+'</span>'+tag(a.isToday,18,10.5)+chips(a)+(isRead(a.id)?'<span style="font-size:10.5px;color:var(--color-label-alternative);">읽음</span>':'')+'</div>'
  ...
  function railCard(a){
    ...
    +'<div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:8px;"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;"><span style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--color-primary-normal);">'+e(a.kicker)+'</span>'+tag(a.isToday,16,10)+chips(a)+'</div>'+bookmarkBtn(a.id,15,'plain')+'</div>'
  ...
  function bandCard(a){
    ...
    +'<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;flex-wrap:wrap;"><span style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--color-primary-normal);">'+e(a.kicker)+'</span>'+tag(a.isToday,16,10)+chips(a)+'</div>'
  ...
  function rowCard(a){
    ...
    +'<div style="flex:1;min-width:0;"><div style="display:flex;align-items:center;gap:6px;margin-bottom:6px;flex-wrap:wrap;"><span style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--color-primary-normal);">'+e(a.kicker)+'</span>'+tag(a.isToday,16,10)+chips(a)+'</div>'
  ...
  // 홈 상단 "보호자 레이더" 스트립 — 곧 보호자가 물어볼 주제를 모아 미리 대비하게.
  function ownerRadarStrip(arr){
    if(S.cat!=='all'||S.unreadOnly||S.query.trim()||DATA.weekly) return '';
    var soon=arr.filter(function(a){return a.radar&&a.radar.owner&&a.radar.owner.soon&&a.radar.owner.q;}).slice(0,8);
    if(soon.length<2) return '';
    var cards=soon.map(function(a){
      return '<div data-open="'+a.id+'" style="flex:none;width:270px;scroll-snap-align:start;cursor:pointer;border:1px solid var(--color-line-normal);border-radius:12px;padding:16px 17px;background:var(--color-background-normal);">'
      +'<div style="font-size:10.5px;font-weight:700;letter-spacing:.04em;color:var(--color-label-alternative);margin-bottom:8px;text-transform:uppercase;">'+e(a.kicker)+' · '+e(a.source)+'</div>'
      +'<div style="font-size:14.5px;font-weight:700;line-height:1.45;color:var(--color-label-strong);">“'+e(a.radar.owner.q)+'”</div>'
      +'<div style="margin-top:10px;font-size:11.5px;font-weight:700;color:var(--color-primary-normal);display:flex;align-items:center;gap:4px;">답변 준비하기 '+ARR+'</div></div>';
    }).join('');
    return '<div style="margin:6px 0 4px;padding:20px 22px;border:1px solid var(--color-primary-normal);border-radius:16px;background:linear-gradient(135deg,rgba(0,102,255,0.05),rgba(0,102,255,0.09));">'
    +'<div style="display:flex;align-items:center;gap:9px;margin-bottom:4px;"><span style="font-size:14px;">⚑</span><span style="font-family:var(--font-display);font-size:18px;font-weight:800;letter-spacing:-.01em;color:var(--color-label-strong);">보호자 레이더 — 곧 이렇게 물어봅니다</span></div>'
    +'<p style="margin:0 0 14px;font-size:12.5px;line-height:1.5;color:var(--color-label-neutral);max-width:640px;">해외에서 지금 뜨는 이슈는 곧 한국 보호자에게도 옵니다. 진료실에서 당황하지 않도록 미리 답을 준비하세요. 각 카드에 설명 대본이 들어 있습니다.</p>'
    +'<div class="vm-radar-scroll" style="display:flex;gap:12px;overflow-x:auto;scroll-snap-type:x mandatory;padding-bottom:4px;">'+cards+'</div></div>';
  }
  ...
  function homeView(){
    ...
    var h='<div>'+ownerRadarStrip(arr)+'<div class="vm-grid" style="display:grid;grid-template-columns:2fr 1fr;gap:0;border-bottom:2px solid var(--color-label-strong);">'
  ...
  function detail(){
    ...
    +'<div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;flex-wrap:wrap;"><span style="font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--color-primary-normal);">'+e(a.kicker)+'</span>'+tag(a.isToday,17,10)+ownerChip(a)+evBadge(a)+'</div>'
  ...
  function render(){
    ...
    h+='<div class="vm-dateline" style="display:flex;align-items:center;justify-content:space-between;padding:12px 0;font-size:12px;color:var(--color-label-alternative);"><span style="flex:1;text-align:left;">'+e(DATA.dateline)+'</span><span style="flex:1;text-align:center;letter-spacing:.06em;">한국 동물병원 해외 조기경보 레이더</span><span style="flex:1;text-align:right;">원문 출처 표기 · 번역 참고용</span></div>';
    h+='<div style="height:1px;background:var(--color-line-normal);"></div>';
    var soonCnt=DATA.articles.filter(function(a){return a.radar&&a.radar.owner&&a.radar.owner.soon;}).length;
    ...
    +'<p style="max-width:600px;margin:16px auto 0;font-size:15px;line-height:1.65;color:var(--color-label-neutral);">해외 수의 미디어·논문 수백 곳을 매일 대신 읽고, <b style="color:var(--color-label-strong);font-weight:700;">한국 동물병원이 오늘 알아야 할 것</b>만 골라 실행형으로 전합니다.<br>번역된 뉴스가 아니라, <b style="color:var(--color-primary-normal);font-weight:700;">내일 진료를 바꿀 신호</b>입니다.</p>'
    +'<div class="vm-radar-legend" style="display:flex;justify-content:center;gap:10px;flex-wrap:wrap;margin:20px auto 0;max-width:720px;">'
    + [['🩺','진료 레이더','뒤처지지 않게 — 진료 점검 포인트'],['⚑','보호자 레이더','당황하지 않게 — 곧 물어볼 것+설명 대본'],['✍️','선점 레이더','앞서가게 — 블로그로 먼저 검색 선점']].map(function(x){
        return '<div style="flex:1;min-width:180px;max-width:230px;text-align:left;padding:12px 14px;border:1px solid var(--color-line-normal);border-radius:12px;background:var(--color-background-normal);"><div style="font-size:12.5px;font-weight:800;color:var(--color-label-strong);margin-bottom:3px;">'+x[0]+' '+x[1]+'</div><div style="font-size:11.5px;line-height:1.45;color:var(--color-label-alternative);">'+x[2]+'</div></div>';
      }).join('')
    +'</div>'
    + (soonCnt&&!DATA.weekly?'<div style="margin:14px auto 0;font-size:12.5px;color:var(--color-label-alternative);">오늘 <b style="color:var(--color-primary-normal);">'+soonCnt+'건</b>이 곧 보호자가 물어볼 주제입니다 — 미리 답을 준비하세요.</div>':'')
    +'</header>';
