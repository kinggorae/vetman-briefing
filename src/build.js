...
@media (max-width:900px){
  ...
  .vm-quadzone,.vm-featzone,.vm-brief-grid{grid-template-columns:1fr !important;}
...
(function(){
  ...
  // 간추린 소식(브리프) — 심층 기사 아래, 2단 촘촘한 목록. 개별 페이지 없이 클릭 시 상세.
  function briefsBoard(){
    if(S.cat!=='all'||S.unreadOnly||S.query.trim()||DATA.weekly) return '';
    var list=DATA.briefs||[]; if(!list.length) return '';
    var rows=list.map(function(a){
      return '<button class="vm-brief-it'+readCls(a.id)+'" data-open="'+a.id+'" style="width:100%;text-align:left;display:flex;gap:10px;align-items:baseline;padding:11px 2px;border:0;border-top:1px solid var(--color-line-normal);background:transparent;cursor:pointer;font-family:inherit;">'
      +'<span style="flex:none;color:var(--color-primary-normal);font-size:12px;padding-top:3px;">•</span>'
      +'<div style="min-width:0;"><div class="vm-hl" style="font-size:14px;font-weight:700;line-height:1.4;color:var(--color-label-strong);text-wrap:pretty;">'+e(a.title)+'</div>'
      +'<div style="margin-top:2px;font-size:11px;letter-spacing:.02em;text-transform:uppercase;color:var(--color-label-alternative);">'+[a.source,a.country].filter(Boolean).map(e).join(' · ')+'</div></div></button>';
    }).join('');
    return '<div style="margin-top:34px;">'
    +'<div style="padding:16px 0 12px;border-bottom:2px solid var(--color-label-strong);"><span style="font-family:var(--font-display);font-size:20px;font-weight:800;letter-spacing:-.01em;color:var(--color-label-strong);">간추린 소식</span><span style="margin-left:9px;font-size:12.5px;color:var(--color-label-alternative);">그 밖에 눈여겨볼 해외 소식 '+list.length+'건</span></div>'
    +'<div class="vm-brief-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:0 32px;">'+rows+'</div></div>';
  }
